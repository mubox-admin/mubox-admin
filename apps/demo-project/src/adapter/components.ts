 ...
 * 可用于 mubox-form、mubox-modal、mubox-drawer 等组件使用,
 */

import type { BaseFormComponentType } from '@mubox/components';

import type { Component, SetupContext } from 'vue';
import { message } from '#/adapter/naive';

import { ApiComponent, globalShareState } from '@mubox/components';
...
function withDefaultPlaceholder<T extends Component>(component: T, type: 'input' | 'select') {
  const placeholderMap = {
    input: $t(`ui.placeholder.input`),
    select: $t(`ui.placeholder.select`),
  };
  return (props: any, { attrs, slots }: Omit<SetupContext, 'expose'>) => {
    const placeholder = props?.placeholder || placeholderMap[type];
