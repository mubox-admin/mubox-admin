...
const verifyTip = computed(() => {
  return state.isPassing
    ? `${$st.value('sliderRotateSuccessTip')}${((state.endTime - state.startTime) / 1000).toFixed(1)}${$st.value('seconds')}`
    : $st.value('sliderRotateFailTip');
});
    ...
    >
      ...
      >
        ...
        <div v-if="!state.dragging" class="bg-black/30">
          {{ defaultTip || $st('ui.captcha.sliderRotateDefaultTip') }}
        </div>
      </div>
    </div>
