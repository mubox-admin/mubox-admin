...
import type { AnyFunction } from '@mubox/types';

import type { Component } from 'vue';

import { MuboxAvatar, MuboxIcon } from '@mubox-core/mubox-ui';

import { useMuboxModal } from '@mubox-core/popup-ui';
import { preferences, usePreferences } from '@mubox-core/preferences';
import {
  Badge,
  DropdownMenu,
  DropdownMenuContent,
...
import { useHoverToggle } from '@mubox/hooks';
import { $t } from '@mubox/locales';
import { useLockStore } from '@mubox/stores';
import { isWindowsOs } from '@mubox/utils';
...
const [LockModal, lockModalApi] = useMuboxModal({
  connectedComponent: LockScreenModal,
});
const [LogoutModal, logoutModalApi] = useMuboxModal({
  onConfirm() {
    handleSubmitLogout();
  },
});
    ...
    <DropdownMenuTrigger ref="refTrigger" :disabled="props.trigger === 'hover'">
      <div class="hover:bg-accent ml-1 mr-2 cursor-pointer rounded-full p-1.5">
        <div class="hover:text-accent-foreground flex-center">
          <MuboxAvatar :alt="text" :src="avatar" class="size-8" dot />
        </div>
      </div>
    </DropdownMenuTrigger>
    <DropdownMenuContent class="mr-2 min-w-[240px] p-0 pb-1">
      <div ref="refContent">
        ...
        <DropdownMenuItem
          ...
          @click="menu.handler"
        >
          <MuboxIcon :icon="menu.icon" class="mr-2 size-4" />
          {{ menu.text }}
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem
