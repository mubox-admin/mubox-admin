import type { Recordable } from '@mubox-core/typings';
...
import type { FormActions, FormSchema, MuboxFormProps } from './types';

import { Store } from '@mubox-core/shared/store';
import {
  bindMethods,
  createMerge,
  isDate,
...
function getDefaultState(): MuboxFormProps {
  return {
    actionWrapperClass: '',
    collapsed: false,
    collapsedRows: 1,
...
export class FormApi {
  // private api: Pick<MuboxFormProps, 'handleReset' | 'handleSubmit'>;
  public form = {} as FormActions;
  isMounted = false;

  public state: null | MuboxFormProps = null;
  stateHandler: StateHandler;

  public store: Store<MuboxFormProps>;

  // 最后一次点击提交时的表单值
  private latestSubmissionValues: null | Recordable<any> = null;

  private prevState: null | MuboxFormProps = null;

  constructor(options: MuboxFormProps = {}) {
    const { ...storeState } = options;

    const defaultState = getDefaultState();

    this.store = new Store<MuboxFormProps>(
      {
        ...defaultState,
        ...storeState,
      },
  ...
  setState(
    stateOrFn:
      | ((prev: MuboxFormProps) => Partial<MuboxFormProps>)
      | Partial<MuboxFormProps>,
  ) {
  ...
  private async getForm() {
    ...
    if (!this.form?.meta) {
      throw new Error('<MuboxForm /> is not mounted');
    }
    return this.form;
  }
