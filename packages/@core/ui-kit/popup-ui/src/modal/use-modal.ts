...
import MuboxModal from './modal.vue';

const USER_MODAL_INJECT_KEY = Symbol('MUBOX_MODAL_INJECT');

export function useMuboxModal<TParentModalProps extends ModalProps = ModalProps>(
...
async function checkProps(api: ExtendedModalApi, attrs: Record<string, any>) {
  ...
  for (const attr of Object.keys(attrs)) {
    if (stateKeys.has(attr) && !['class'].includes(attr)) {
      ...
      console.warn(
        `[Mubox Modal]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Modal, please use useMuboxModal or api.`,
      );
    }
  }
}
