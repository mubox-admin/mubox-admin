...
import MuboxDrawer from './drawer.vue';

const USER_DRAWER_INJECT_KEY = Symbol('MUBOX_DRAWER_INJECT');

export function useMuboxDrawer<
...
>(options: DrawerApiOptions = {}) {
  ...
  const Drawer = defineComponent(
    (props: DrawerProps, { attrs, slots }) => {
      return () =>
        h(MuboxDrawer, { ...props, ...attrs, drawerApi: extendedApi }, slots);
    },
    {
      inheritAttrs: false,
      name: 'MuboxDrawer',
...
async function checkProps(api: ExtendedDrawerApi, attrs: Record<string, any>) {
  ...
  for (const attr of Object.keys(attrs)) {
    if (stateKeys.has(attr) && !['class'].includes(attr)) {
      // connectedComponent存在时，不要传入Drawer的props，会造成复杂度提升，如果你需要修改Drawer的props，请使用 useMuboxDrawer 或者api
      console.warn(
        `[Mubox Drawer]: When 'connectedComponent' exists, do not set props or slots '${attr}', which will increase complexity. If you need to modify the props of Drawer, please use useMuboxDrawer or api.`,
      );
    }
  }
}
